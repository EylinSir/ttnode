...
if($_REQUEST)  
{  
	if($_REQUEST['phone'] && !empty($_REQUEST['cz']) && !empty($_REQUEST['phone']) ){
	$cz   = $_REQUEST['cz'];
	$yzm   = $_REQUEST['yzm'];	
	 $phone   = $_REQUEST['phone'];	
    if($cz=='yzm')
    {  
        $curl='curl -X POST http://tiantang.mogencloud.com/web/api/login/code?phone='.$phone;
		$zone_id0=json_decode(exec($curl), true);
		print_r($zone_id0);
		if($zone_id0['errCode']==0){echo"<br />验证码发送成功";}
    }  
     if($cz=='cookie')
    {  
$curl='curl -X POST http://tiantang.mogencloud.com/web/api/login?phone='.$phone.'\&authCode='.$yzm;
$zone_id0=json_decode(exec($curl), true);
//print_r($zone_id0);
echo "<br />".$zone_id0['msg'];
echo "<br />账&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;号：".$zone_id0['data']['nickName'];
echo "<br />手&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;机：".$zone_id0['data']['phoneNum'];
echo "<br />剩余星愿：".$zone_id0['data']['score'];
echo "<br />历史星愿：".$zone_id0['data']['add_up_score'];
echo "<br />T&nbsp;o&nbsp;k&nbsp;e&nbsp;n：".$zone_id0['data']['token'];
    }  
	}
}else{
		?>
...
#background-3{
    background: url("https://api.ixiaowai.cn/api/api.php?a=<?php echo date("Y-m-d h:i:sa")?>") no-repeat;
        ...
        <form action="" method="post">
            <div class="flrg">
                ...
                <div class="a" style='height:50px;'>
                    <input type="text" autocomplete="off" class="in-1" id='phone' name='phone'  value=''  placeholder="手机号码" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" >
					
                </div>
				<div class="a" style='height:50px;'>
<input class='in-1' type="button" onclick="yzm()" value="获取手机验证码" onmouseover="this.style.backgroundColor='green';" onmouseout="this.style.backgroundColor='';"  />

</div>
                <div class="a" style='height:50px;'>
                    <input type="text" class="in-1" autocomplete="off" id='phone_yzm' name='phone_yzm'  value=''   placeholder="输入手机验证码" >
					
                </div>
				<div class="a" style='height:50px;'>
<input class='in-1' type="button" onclick="cookieto()" value="获取Cookie" onmouseover="this.style.backgroundColor='green';" onmouseout="this.style.backgroundColor='';"  />
                </div>
    ...
    <div class="box-2 le-2">
            ...
            <div class="flrg-1" style='text-align:left;word-break:pre-wrap;'>
			<center><h3>甜糖信息</h3></center>
                <div class="a" style='text-align:left;height:50px;word-break:pre-wrap;' >
                   <div id="yzm_log" style='max-width:100%;text-align:left;height:100px;overflow:hidden;word-break: break-all;word-wrap: break-word;' ></div>
                </div>
				<div class="a" style='text-align:left;height:300px;word-break:pre-wrap;'>
                    <div id="cookie_log" style="max-width:100%;text-align:left;height:300px;overflow:hidden;word-break: break-all;word-wrap: break-word;" ></div>
                </div>
            </div>
    </div>
...
$(document).ready(function() {  
  ...
  function yzm(){   
        ...
        $.post("ttnode.php", {	
            ...
            $('#yzm_log').fadeOut();  
             $('#Loading').hide();  
            setTimeout("finishAjax('yzm_log', '"+escape(response)+"')", 100);  
        });  
}
  function cookieto(){    
        $('#Loading').show();  
        $.post("ttnode.php", {	
            ...
            $('#cookie_log').fadeOut();  
             $('#Loading').hide();  
            setTimeout("finishAjax('cookie_log', '"+escape(response)+"')", 100);  
        });  	
}
